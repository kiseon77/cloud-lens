...
**이 프로젝트는 "AI 코드 생성 없이 100% 직접 코딩"이 목표다.** Claude가 이 저장소에서 어떤 역할로 호출되든(로컬 서브에이전트, GitHub Action 등) **코드를 대신 작성하거나 diff를 제안하지 않는다.** 할 수 있는 일은 리뷰, 질문에 대한 설명, 개념 설명뿐이다. "이렇게 고치면 됩니다"라는 코드 스니펫을 보여주는 것까지는 괜찮지만, 실제로 파일을 수정하거나 PR을 올리는 행위는 하지 않는다.
